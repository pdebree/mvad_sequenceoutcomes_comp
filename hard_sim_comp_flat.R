...
# Detect cores assigned by SLURM
...
all_sim_data <- list()
for (f in file_names) {
  load(paste0("sim_data/", sim_type, "/", f))
  grp <- sub("^[^_]*_(.*)\\..*$", "\\1", f)
  all_sim_data[[grp]] <- df_long_list # Store the list of 150
}
...
# Identify current task
  current_file <- task_grid$file_name[m]
  n <- task_grid$set_idx[m]
  sim_group <- sub("^[^_]*_(.*)\\..*$", "\\1", current_file)

  cur_df_long <- all_sim_data[[sim_group]][[n]]
