...
results <- foreach(m = 1:nrow(task_vec), .packages = c("tidyverse", "cluster", "TraMineR", "cfda", "TraMineRextras")) %dopar% {
  ...
  p1 <- colnames(mvad_states)[2:19]
  p2 <- colnames(mvad_states)[20:37]
  ...
  for (i in 1:folds) {
    ...
    pca_windows_train <- prcomp(x=mvad_windows[12:22][train_idx,], center=TRUE, scale=TRUE)
    train_scores <- pca_windows_train$x
    test_scores <- predict(pca_windows_train, newdata = mvad_windows[12:22][test_idx, ]) 
...
saveRDS(cv_comp, file="CV_LinearComp_halfperiod.rds")
