...
fit_rf <- function(train_data, test_data, mtry = NA) {
...
hard_cluster <- function(clusterward, nClusts, covars, y, train_idx, test_idx, dist_matrix) {
  
  cut1 <- cutree(clusterward,k=nClusts)
  clust1.fac <- factor(cut1)
  train_data <- covars[train_idx,] %>% 
    mutate(cluster=clust1.fac, y=y[train_idx]) 
  
  cut2 <- assign_new(dist_matrix,train_idx,cut1,test_idx)
  clust2.fac <- factor(cut2)
  test_data <- covars[test_idx,] %>% mutate(cluster=clust2.fac, y=y[test_idx])
...
hard_cluster_onehot <- function(clusterward, nClusts, covars, yr, train_idx, test_idx, dist_matrix) {
  ...
  cut1 <- cutree(clusterward,k=nClusts)
  clust1.fac <- factor(cut1)
  train_data <- covars[train_idx,] %>% mutate(cluster=clust1.fac, y=y[train_idx])
  
  # Create dummy columns for all expected clusters
  for(col in all_clusters) {
    train_data[[col]] <- as.integer(train_data$cluster == sub("cluster_", "", col))
  }
  ...
  # Create dummy columns for all expected clusters (can be empty )
  for(col in all_clusters) {
    test_data[[col]] <- as.integer(test_data$cluster == sub("cluster_", "", col)) 
  }
