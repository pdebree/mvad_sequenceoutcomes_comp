...
results <- foreach(m = 1:nrow(task_vec), .packages = c("tidyverse", "cluster", "TraMineR", "cfda", "TraMineRextras")) %dopar% {
  ...
  p1 <- colnames(mvad_states)[2:7]
  p2 <- colnames(mvad_states)[8:13]
  p3 <- colnames(mvad_states)[14:19]
  p4 <- colnames(mvad_states)[20:25]
  p5 <- colnames(mvad_states)[26:31]
  p6 <- colnames(mvad_states)[32:37]


  # makes year and id combinations with counts for each state
  id_period <- mvad_states %>% pivot_longer(cols=colnames(mvad_states[2:37]), names_to="output") %>% 
    mutate(period = case_when(
      ...
      output %in% p4 ~ 4, 
      output %in% p5 ~ 5, 
      TRUE ~ 6)) %>% count(id, period, value)

  # make full table with 16 columns (one for each year/id combo) + 1 for id.
  mvad_states_wide <- id_period %>% 
    pivot_wider(id_cols=c(id), 
                names_from=c(value,period), values_from=n, values_fill = 0) %>% dplyr::select(-id)
  ...
  for (i in 1:folds) {
    ...
    pca_windows_train <- prcomp(x=mvad_windows[12:43][train_idx,], center=TRUE, scale=TRUE)
    train_scores <- pca_windows_train$x
    test_scores <- predict(pca_windows_train, newdata = mvad_windows[12:43][test_idx, ]) 
...
saveRDS(cv_comp, file="CV_LinearComp_sixmonths.rds")
