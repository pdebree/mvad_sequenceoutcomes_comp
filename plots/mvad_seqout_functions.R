...
assign_new_soft <- function(dist_mat,train.idx,train.memb,test.idx,memb.exp=1.5,n.draws=300,prior.n.equiv=100) {
  
  len <- length(train.idx) # all members 
  simmat <- 1-dist_mat/max(dist_mat) # all similarities 
  test_locs <- (1:len)[test.idx] # indicies of test locations
...
train_test_lm_clust_soft <- function(dists, nClusts, dat, Y.cont, train_idx,test_idx, fuzziness=2) {
